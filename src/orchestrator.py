...
def request_human_approval(task_id: str, user_id: str, tool_name: str, arguments: str):
    ...
    logger.warning(
        f"[HITL PENDING] Intercepted high-risk execution '{tool_name}' on Task ID: {task_id}."
    )
...
def execute_agent_tool(user_id: str, tool_name: str, arguments: str) -> str:
    ...
    try:
        ...
        # --- 3. EXTERNAL WEBHOOKS ---
            ...
            if not target_url:
                return (
                    "ERROR: EXTERNAL_WEBHOOK_URL is not configured in the environment."
                )
        ...
        if tool_name in HIGH_RISK_TOOLS:
            ...
            with tracer.start_as_current_span("hitl_human_checkpoint") as span:
                ...
                if not wait_for_approval(task_id):
                    ...
                    dag.add_step(
                        "HumanSupervisor", "AgentOrchestrator", "Execution DENIED"
                    )
        ...
        with tracer.start_as_current_span("sandbox_execution") as span:
            ...
            dag.add_step(
                "ToolExecutor", "FirecrackerMicroVM", "Booting Hardware Sandbox"
            )
            ...
            try:
                ...
                dag.add_step(
                    "FirecrackerMicroVM", "AgentOrchestrator", "Tool Execution Success"
                )
                return result
            except Exception as e:
                span.set_status(trace.StatusCode.ERROR, description=str(e))
                dag.add_step(
                    "FirecrackerMicroVM", "AgentOrchestrator", "Tool Execution FAILED"
                )
                return (
                    f"ERROR: Tool execution failed inside secure boundary. Reason: {e}"
                )
            finally:
                vm.teardown()
